  ...
  if (!req.session.userinfo) {
    models.sequelize.Promise.all([
      ...
      .spread((returnNotice, returnFAQ, returnCommunity) => {
        return res.status(200).render('common/index', {
          notice: returnNotice,
          faq: returnFAQ,
    ...
    models.sequelize.Promise.all([
      ...
      .spread(returnCommunity => {
        if (req.session.userinfo[1] === student) {
          return res.render('student/index', { community: returnCommunity });
        } else {
          return res.render('professor/index', { community: returnCommunity });
        }
      })
      .catch(function(err) {
        return res.status(500).redirect('/');
